...
- Seguridad: auth/authz negativa, rate limiting, webhooks, dependencias; H2 agrega `security-check` para secretos/configuracion y auditoria frontend critica.
...
| H2 - Seguridad y secretos | Reducir riesgos de auth, permisos, secretos y endpoints sensibles. | auth/authz, secretos, CORS, headers, rate limiting, webhooks, pagos, dependencias. | redise?o multi-tenant completo, MFA salvo decision explicita. | H1, owners secretos, politica seguridad. | CI minima disponible. | mini-spec, threat model, inventario de secretos, `security-check`, pruebas de settings/logging y evidencia remota. | cambios de auth pueden afectar sesiones; escaneo puede requerir allowlist minima. | pytest, OpenAPI, permisos, logs sin secretos, secret scan, config check, npm audit critico. | `chore(security): harden secrets and application boundaries` |
